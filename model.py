...
class res_DW_3DCNN(nn.Module):
    def __init__(self):
        super().__init__() # input:(B,C,64,112,112)
        self.conv1 = nn.Conv3d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.norm = nn.GroupNorm(8, 32)
        self.dwconv = nn.Conv3d(32, 32, kernel_size=3, padding=1, groups=32)  # dw

        self.cbam1 = CBAMModule3D(32)
        self.pwconv = nn.Conv3d(32, 3, kernel_size=1, stride=1, padding=0)  # pointwise/1x1 convs
        self.act = nn.GELU()
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        # 3DCNN
        self.res_DW_3DCNN = res_DW_3DCNN()
